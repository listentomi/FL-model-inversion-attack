"""
Federated CIFAR-10 dataset and client population class
"""
...
class Population():
    def __init__(self,
                 ...
                 precomputed=True,
                 embedding_fname='embeddings-d=cifar10-e=4-st=5-lr=0.001-mo=0.9-wd=0.0001-fc2-train.npy',
                 test_embedding_fname='embeddings-d=cifar10-e=4-st=5-lr=0.001-mo=0.9-wd=0.0001-fc2-test.npy',
                 shuffle=True):

        self.dataset_root = './data/cifar10'
    ...
    def load_dataset(self):
        """
        Load CIFAR-10 dataset from torchvision.datasets.CIFAR10
        """
        ...
        if args.arch in ['tf_cnn', 'base_cnn', 'split']:
        ...
        if exists('./data/cifar10/cifar-10-batches'):
            download = False
            print_header(f'> Loading data from {self.dataset_root}...')
        else:
            download = True
        self.train_data = torchvision.datasets.CIFAR10(root=self.dataset_root, train=True,
                                                       download=download, transform=self.transform)
        self.test_data = torchvision.datasets.CIFAR10(root=self.dataset_root, train=False,
                                                      download=download, transform=self.test_transform)
    ...
    def finalize_client_datasets(self):
        ...
        if args.num_local_train_pooled != 0.:  # Can probably remove this
            for client in self.clients:
                pooled_data_len = int(len(client.datasets[0]) *
                                      args.num_local_train_pooled / len(self.clients))
                saved_data_len = len(client.datasets[0]) - pooled_data_len
                # Split client's original validation split into split to keep and split to share
                pooled_data_split = random_split(client.datasets[0],
                                                 [saved_data_len, pooled_data_len])
                # Save the split to share
                pooled_train_data.append(pooled_data_split[1])
                ...
                saved_data_len = int(len(pooled_data_split[0]) *
                                     client.local_val_ratio)

                datasets = random_split(pooled_data_split[0],
                                        [saved_data_len, len(pooled_data_split[0]) - saved_data_len])
                client.datasets[0] = datasets[0]
            # Then add the pooled data back to each client
            for ix, client in enumerate(self.clients):
                if args.local_val_model_delta_ratio:  # try setup where evaluate on both and take the average
                    client.datasets.append(ConcatDataset(pooled_train_data))
                else:
                    client_train_set = [client.datasets[0]]
                    all_data_counts = [len(x.indices) for x in pooled_train_data]

                    if client.shared_val:
                        client_train_set.extend(pooled_train_data)

                    client.datasets[0] = ConcatDataset(client_train_set)

            print_header('Client finalized dataset sizes')
            for ix, dist in enumerate(self.distributions):
                for cix, client in enumerate(dist['clients']):
                    print_debug(f'train: {len(client.datasets[0])}, val: {len(client.datasets[1])}, test: {len(client.datasets[2])}', f'Dist {ix} client {cix} dataset size')
                print('')
            return
